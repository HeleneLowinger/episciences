$(function () {
    ...
    $specialIssuesAccessCode.on('input propertychange', function () {
        ...
        if (access_code.trim() !== '') {
            ...
            request.done(function (result) {
                if (result.status === 1) {
                    button.html('<span class="glyphicon glyphicon-ok green"></span>');
                    valid_code(result);
                } else {
                    if ($specialIssuesAccessCode.val() !== '') {
                        button.html('<span class="glyphicon glyphicon-remove red"></span>');
                    }
    ...
    function valid_code(data) {
        ...
        let volumes_options = '<option value="' + data.vid + '">' + data.volume + '</option>';
        let isSelected = data.allEditorsSelected;
        if ($volumes.length) {
            $("#volumes").html(volumes_options);
        } else {
            $specialIssuesAccessCodeElement.after(createVolumesElement(volumes_options));
        }

        // create editor selection menu
        if (data.canPickEditor > 0) { // contributors can choose editors
            ...
            let suggest_editors_options = createSuggestEditorOptions(data.editors, isSelected);
            ...
            if ($suggest_editors.length) {
                ...
                $('#disclaimers-disclaimer1-element').before(createSuggestEditorsElement(label, suggest_editors_options));
            }
        }
    }
            ...
            if (result['status'] === 2) {
                if ('newVerErrors' in result) {
                    ...
                    if (newVersionErrors.canReplace) {
                        ...
                        $submit_form.append('<input id = "old_repoid" type="hidden" name="old_repoid" value="' + newVersionErrors.oldRepoId + '">');
                        $submit_form.append('<input id = "old_paper_status" type="hidden" name="old_paper_status" value="' + newVersionErrors.oldPaperStatus + '">');
                        $submit_form.append('<input id = "old_paper_sid" type="hidden" name="old_paper_sid" value="' + newVersionErrors.oldSid + '">');
                        $submit_form.append('<input id = "old_paper_vid" type="hidden" name="old_paper_vid" value="' + newVersionErrors.oldVid + '">');
                        if (newVersionErrors.oldPaperId) {
                            $submit_form.append('<input id= "old_paperid" type="hidden" name="old_paperid" value="' + newVersionErrors.oldPaperId + '">');

                        }
                        if (newVersionErrors.submissionDate) {
                            $submit_form.append('<input id = "old_submissiondate" type="hidden" name="old_submissiondate" value="' + newVersionErrors.submissionDate + '">');
                        }
                    }
                    ...
                    let hideResultMessage = (function hideResultMessage() {
                        ...
                        $('#submitForm').fadeIn();
                        $("#form_required").show();
                        applyAction(['specialIssueAccessCode-element', 'volumes-element', 'sections-element', 'suggestEditors-element'], 'hide');
                    });

                    message = '<div id="result_message" class="panel panel-danger">';
    ...
    function resetVolumesElement(data) {
        ...
        if (data.canChooseVolumes) {
            $('#specialIssueAccessCode-element').after(createVolumeMenu(data.volumesOptions));
        }
    }
    ...
    function resetSuggestEditorsElement(data) {
        ...
        if (canPickEditor > 0 && $suggest_editors.length) {
            ...
            $("label[for*='suggestEditors']").text(translate("Je souhaite que mon article soit supervisé par :"));
            $suggest_editors.html(createSuggestEditorOptions(data.editors, data.allEditorsSelected));
...
function createVolumesElement(options) {
    return '<div id="volumes-element" class="form-group row">' +
        '<label class="col-md-3 control-label optional" for="volumes">' + translate("Proposer dans le volume :") + '</label>' +
        '<div class="col-md-9">' +
        '<select class="form-control input-sm" style="width:33%" id="volumes" name="volumes">' + options + '</select>' +
        '</div>' +
        '</div>';
}
...
function createSuggestEditorsElement(label, suggestEditorsOptions) {
    return '<div id="suggestEditors-element" class="form-group row">' +
        '<label class="col-md-3 control-label required" for="suggestEditors">' + translate(label) + '</label>' +
        '<div class="col-md-9">' +
        '<select class="form-control" multiple="multiple" id="suggestEditors" name="suggestEditors[]">' + suggestEditorsOptions + '</select>' +
        '</div>' +
        '</div>';
}
